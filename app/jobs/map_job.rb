...
class MapJob < ApplicationJob
  ...
  def perform(routing_id)
    ...
    iterations.times do |_iteration|
      ...
      destinations += [proposed_destination]
      ...
      if response_data.present?
        response_data['routes'].each do |route|
          ...
          next unless !distance.present? || (distance > tmp_distance)

          distance = tmp_distance
          duration = tmp_duration
          routing_output = (
            [origin] +
            route['waypoint_order'].map do |order|
              destinations[order]
            end +
      ...
      break if @routing_info.status != RoutingInfo::STATUS_COMPLETE
    end
  ...
  def get_map_data(origin, destinations)
    ...
    when 500...600
      Rails.logger.info 'Server side internal error.' \
