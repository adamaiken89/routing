...
class RoutingInfo < ApplicationRecord
  ...
  JOB_STATUSES = %w[processing failed net_error complete].freeze
  ...
  def format_of_routing_info
    if routing_input.present?
      # Do Nothing
    else
      errors.add(
        :routing_input,
        'Please specify both the origin and desintations.'
      )
    end
  end

  def format_of_routing_info_origins
    origin = routing_input.try(:first)
    if origin && origin.size == 2 &&
       origin[0].try(:numeric?) && origin[1].try(:numeric?)
      # Do Nothing
    else
      errors.add(
        :routing_input,
        'Origin should be with numeric latitude and longitude only' \
        'having double quotes around.'
      )
    end
  end

  def format_of_routing_info_destinations
    destinations = routing_input[1..-1]

    if destinations.present? && !destinations.empty?
      if destinations.size > 24
        errors.add(
          :routing_input,
          'Destinations should be input with at most 24 points.'
        )
        return
      end
      ...
      destinations.each do |destination|
        if destination.size == 2 &&
           destination[0].try(:numeric?) &&
           destination[1].try(:numeric?)
          # Do Nothing
        else
          errors.add(
            :routing_input,
            'Destinations should be with numeric latitude and longitude only' \
            'having double quotes around.'
          )
        end
      end
      ...
      errors.add(
        :routing_input,
        'Please specify at least one desintations.'
      )
    end
  end
end
